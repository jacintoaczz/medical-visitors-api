...
public interface AppointmentRepository extends JpaRepository<Appointment, Long> {
	...
	@Query(value = "SELECT IFNULL(COUNT(visitors.id), 0) as num_appointments FROM appointments\r\n"
			...
			+ "WHERE visitors.company = ?1 "
			+ "AND hospitals.email = ?2"
			+ "AND date_format(appointments.date, '%Y-%m') = date_format(?3, '%Y-%m') "
			+ "AND appointments.is_accepted = true\r\n"
			+ "group by date_format(appointments.date, '%Y-%m')", nativeQuery = true)
	Long countVisitorsOnMonthByHospitalAndByCompany(String email,String companyName, Date date);
